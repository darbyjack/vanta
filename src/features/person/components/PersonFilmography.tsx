...
export function PersonFilmography({ credits }: { credits: TmdbCombinedCredit[] }) {
        ...
        {items.map((credit) => {
          ...
          return (
            <Link
              ...
              className="grid gap-2 p-3 hover:bg-muted/50 sm:grid-cols-[80px_1fr_auto]"
            >
              <span className="text-sm text-muted-foreground">{yearFrom(creditDate(credit)) ?? "Soon"}</span>
              <span>
                <span className="font-medium">{title}</span>
                {credit.character || credit.job ? (
                  <span className="ml-2 text-sm text-muted-foreground">as {credit.character ?? credit.job}</span>
                ) : null}
              </span>
