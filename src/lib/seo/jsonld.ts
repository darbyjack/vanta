...
import type { TmdbMovieDetail, TmdbPersonDetail, TmdbTvDetail } from "@/lib/tmdb/types";
...
export function movieJsonLd(movie: TmdbMovieDetail, url: string) {
  return {
    ...
    actor: movie.credits?.cast.slice(0, 8).map((actor) => ({ "@type": "Person", name: actor.name })),
    director: movie.credits?.crew
      .filter((crew) => crew.job === "Director")
      .map((director) => ({ "@type": "Person", name: director.name })),
    aggregateRating: movie.vote_count
      ? { "@type": "AggregateRating", ratingValue: movie.vote_average, ratingCount: movie.vote_count, bestRating: 10 }
      : undefined,
  };
}
...
export function tvJsonLd(tv: TmdbTvDetail, url: string) {
  return {
    ...
    actor: tv.credits?.cast.slice(0, 8).map((actor) => ({ "@type": "Person", name: actor.name })),
    aggregateRating: tv.vote_count
      ? { "@type": "AggregateRating", ratingValue: tv.vote_average, ratingCount: tv.vote_count, bestRating: 10 }
      : undefined,
  };
}
